...
findNcgSupervisedByLinearMixedModel <- function(
    ...
    if (isTRUE(adjust.data)){
        if (adjustment.variables == 'uv'){
            ...
            adjustment.form <- changeLmmFormula(
                form = form,
                out.put = 'sub.formula',
                sub.set = uv.variables
                )
            form <- changeLmmFormula(
                form = form,
                out.put = 'sub.formula',
                sub.set = bio.variables
                )
        ...
        if (adjustment.variables == 'both'){
            ...
            all.lmm <- cbind(
                all.lmm.bio.adjusted[[assay.name]],
                all.lmm.uv.adjusted[[assay.name]]
                )
        }
    }
    ...
    if (isFALSE(use.rank)){
        ...
        if (ncg.selection.method == 'quantile'){
            if (!is.null(uv.percentile) & !is.null(bio.percentile)){
                var.partition <- var.partition[var.partition$tech.var > quantile(x = var.partition$tech.var, probs = uv.percentile) , ]
                if (quantile(x = var.partition$bio.var, probs = bio.percentile) == 0){
                    var.partition <- var.partition
                } else {
                    var.partition <- var.partition[var.partition$bio.var < quantile(x = var.partition$bio.var, probs = bio.percentile) , ]
                }
                nb.ncg <- round(x = nrow(se.obj) * nb.ncg, digits = 0)
                ncg.selected <- row.names(se.obj) %in% var.partition$gene[1:nb.ncg]
