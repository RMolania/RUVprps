...
#' @param assay.name Character. A character used to specify the assay name within the SummarizedExperiment object.
...
#' @param uv.variables Character. A character or character vector representing the label of unwanted variable(s),
...
#' @param bio.clustering.method Character. A character indicating the clustering method used to group each
...
#' @param other.uv.clustering.method Character. A character indicating the clustering method used to group each
...
createPrPsSupervised <- function(
    ...
    if (length(categorical.uv) > 0) {
        printColoredMessage(
            message = '-- Creating PRPS data for all categorical sources of unwanted variation:',
            color = 'magenta',
            verbose = verbose
        )
        if (!save.se.obj) {
