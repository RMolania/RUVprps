...
findNcgByAnovaCorr <- function(
    ...
    if(approach == 'AnovaCorr.AcrossAllSamples' | approach == 'both'){
        printColoredMessage(message = '------------The findNcgByAnovaCorr function starts:',
                            color = 'white',
                            verbose = verbose)
        ...
        if (isFALSE(use.rank)){
            ...
            all.stats$bio <- rowSums(all.stats[ , bio.variables, drop = FALSE])
            all.stats$uv <- rowSums(all.stats[ , uv.variables, drop = FALSE])
            all.stats$ratio <- all.stats$uv / c(all.stats$bio + 1e-6)
            high.tech.genes <- all.stats[order(-all.stats$ratio), ]

            if (ncg.selection.method == 'quantile'){
                high.tech.genes <- high.tech.genes[high.tech.genes$uv > quantile(x = all.stats$uv, probs = uv.percentile) , ]
                high.tech.genes <- high.tech.genes[high.tech.genes$bio < quantile(x = all.stats$bio, probs = bio.percentile) , ]
    ...
    if(approach == 'AnovaCorr.PerBatchPerBiology'){
        ...
        if (isFALSE(use.rank)){
            ...
            all.stats <- as.data.frame(do.call(cbind, all.stats))
            row.names(all.stats) <- row.names(se.obj)
            colnames(all.stats) <- sub("\\|\\|.*", "", colnames(all.stats))
            matched.cols.bio <- grep(paste(bio.variables, collapse = "|"), colnames(all.stats))
            matched.cols.uv <- grep(paste(uv.variables, collapse = "|"), colnames(all.stats))

            all.stats$bio <- rowSums(all.stats[ , matched.cols.bio, drop = FALSE])
            all.stats$uv <- rowSums(all.stats[ , matched.cols.uv, drop = FALSE])
            all.stats$ratio <- all.stats$uv / c(all.stats$bio + 1e-6)
            high.tech.genes <- all.stats[order(-all.stats$ratio), ]
