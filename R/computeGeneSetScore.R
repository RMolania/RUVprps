#' Compute sample level enrichment score of a gene set.
#'
...
computeGeneSetScore <- function(
    ...
    if (isTRUE(save.se.obj)){
        ...
        printColoredMessage(
            message = '------------The computeGeneSetScore function finished.',
            color = 'white',
            verbose = verbose
            )
        return(se.obj)
    ...
    if (isFALSE(save.se.obj)){
        ...
        return(list(all.scores = all.scores))
    }
}
...
# }
#' assess.score Logical. If `TRUE`, the association between the computed scores and specified variables will be
#' assessed. The default is set to `FALSE`. See the details for more information.
#' variables.to.assess Character. A character string or vector of strings indicating the name(s) of the column(s)
...
#' corr.method Character. A character string indicating which correlation method should be used for the correlation
