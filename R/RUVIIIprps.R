...
#' @param assay.name Character string. A character string indicating the name of the assay in the SummarizedExperiment
...
#' @param prps.group Character string. A character string indicating the name of the PRPS group under the 'prps.type' slot.
#' @param prps.set.names Character string specifying the exact name of a PRPS set under the 'prps.group' name in the
...
#' set to 'TRUE'.
#' @param data.to.log Character string. A character string specifying the data that must be log-transformed before applying
...
#' @param eta Character string. Gene-wise (as opposed to sample-wise) covariates. These covariates are adjusted for by RUV-1
#' before any further analysis proceeds. Can be either (1) a matrix with n columns, (2) a matrix with n rows, (3) a data
#' frame with n rows, (4) a vector or factor of length n, or (5) simply 1, for an intercept term.
#' @param include.intercept Logical. When eta is specified (not NULL) but does not already include an intercept term, this will
#' automatically include one.
#' @param return.info Logical. If 'TRUE', additional information including the W matrix of the RUV-III method is returned. The
