...
#' @param ncg.group Character. A character that specifies the name of the group of NCGs stored under the 'supervised' or
...
assessNCGs <- function(
    ...
    if (is.null(ncg.type) | is.null(ncg.group)){
        stop('The "ncg.type" and "ncg.group" must be provided.')
    }
    ...
    if (!is.character(ncg.group) | length(ncg.group) > 1){
        stop('The "ncg.type" must be a single character.')
    }
    ...
    printColoredMessage(
        message = '-- Obtaining specified NCG sets:',
        color = 'magenta',
        verbose = verbose
        )
    ...
    if (ncg.type == 'supervised'){
        printColoredMessage(
            message = '- Obtaining specified NCG sets selected by the supervised approaches:',
            color = 'blue',
            verbose = verbose
            )
    ...
    if (ncg.type == 'un.supervised'){
        printColoredMessage(
            message = '- Obtaining specified NCG sets selected by the un supervised approaches:',
            color = 'blue',
            verbose = verbose
            )
        if (ncg.set.names == 'all'){
    ...
    if (isTRUE(save.se.obj)){
        if (ncg.group %in% c('supervised', 'un.supervised')){
            ...
            if (!is.null(p.assess.ncg)){
                se.obj@metadata[['NCG']][[ncg.type]][[ncg.group]][['assessment.plot']][[output.name]]<- p.assess.ncg
            }
            if (!is.null(p.comparing.ncgs)){
                se.obj@metadata[['NCG']][[ncg.type]][[ncg.group]][['assessment.plot']][[output.name]]<- p.comparing.ncgs
            }
            return(se.obj)
        }
